...
const TrainingsPage = () => {
  return (
    <section id="our-services" className="w-full bg-white text-gray-900">
      {/* Hero / Intro */}
      ...
      <ScrollAnimation animation="fadeInUp">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          <div className="rounded-2xl p-6 border bg-gradient-to-tr from-slate-50 to-white shadow-sm">
            <h3 className="text-xl font-semibold text-slate-900">UX / Design</h3>
            <p className="text-sm text-slate-600 mt-2">
              Bring ideas to digital life: prototyping, design systems, accessibility and user testing.
            </p>
            <Link to="/trainings/ux-ui" className="mt-4 inline-block text-sm font-medium text-sky-600">
              + Learn More
            </Link>
          </div>

          <div className="rounded-2xl p-6 border bg-gradient-to-tr from-slate-50 to-white shadow-sm">
            <h3 className="text-xl font-semibold text-slate-900">Managed IT Services</h3>
            <p className="text-sm text-slate-600 mt-2">
              IT consulting, project delivery and ongoing managed support to keep systems healthy and performant.
            </p>
            <Link to="/trainings/managed-it" className="mt-4 inline-block text-sm font-medium text-sky-600">
              + Learn More
            </Link>
          </div>

          <div className="rounded-2xl p-6 border bg-gradient-to-tr from-slate-50 to-white shadow-sm">
            <h3 className="text-xl font-semibold text-slate-900">Disaster Recovery</h3>
            <p className="text-sm text-slate-600 mt-2">
              Policies, tooling and procedures to ensure rapid recovery and business continuity.
            </p>
            <Link to="/trainings/disaster-recovery" className="mt-4 inline-block text-sm font-medium text-sky-600">
              + Learn More
            </Link>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {courses.map((course, i) => (
            <ScrollAnimation key={course.id} animation={i % 2 === 0 ? "fadeInLeft" : "fadeInRight"}>
              <article className="group relative rounded-2xl border p-6 bg-white shadow hover:shadow-lg transition">
                <div className="flex items-start justify-between">
                  <h3 className="text-xl font-semibold text-slate-900">{course.title}</h3>
                </div>

                <p className="text-sm text-slate-600 mt-3">{course.short}</p>
{/* 
                <p className="text-sm text-slate-700 mt-4 leading-relaxed">{course.detail}</p> */}
      ...
      <ScrollAnimation animation="fadeInUp">
        <div className="max-w-7xl mx-auto px-6 py-16 text-center bg-gradient-to-r from-slate-50 to-white rounded-2xl">
          ...
          <div className="flex items-center justify-center gap-4">
            <Link to="/contact" className="px-6 py-3 rounded-full bg-slate-900 text-white font-semibold">
              Contact Us
            </Link>
            <Link to="/trainings/catalog" className="px-6 py-3 rounded-full border border-slate-200 text-slate-900">
              View Full Catalog
            </Link>
          </div>
        </div>
