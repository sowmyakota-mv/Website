...
const careerSteps = [
  { title: "Explore the Program", icon: FileSearch },
  ...
  { title: "Work on Real-World Projects", icon: FolderKanban },
  { title: "Industry-Ready Training & Mentorship", icon: Presentation },
  { title: "Professional CV Development", icon: FileCheck },
  { title: "Strategic CV Promotion", icon: UserPlus },
  { title: "Career Mentoring & Grooming", icon: UserCheck },
  { title: "Guaranteed Job Placement", icon: Award },
  { title: "Ongoing Career Support", icon: Handshake },
];
...
const CareerCarousel = () => {
  ...
  // 🆕 Handle card click (jump to that card + continue auto-scroll)
  const handleCardClick = (idx) => {
    setCurrent(idx);
    setIsShiftOut(false);

    // Smooth scroll to the carousel section (optional)
    const section = document.getElementById("career-carousel");
    if (section) {
  ...
  return (
    <div
      ...
      style={{ paddingBottom: CARD_HEIGHT / 2 }}
    >
      ...
      >
        ...
        >
          {careerSteps.map((step, idx) => {
            ...
            // --- LEFT SIDE CARDS ---
            if (pos < 0 && !isShiftOut) {
              let scale = pos === -1 ? 1.0 : 0.9 + pos * 0.05;
              const zIdx = 100 + pos;
              ...
              return (
                <div
                  key={idx}
                  onClick={() => handleCardClick(idx)} // ✅ Added click
                ...
                >
                  <div
                    ...
                    style={{
                      boxShadow:
                        pos === 0 ? "0 8px 48px #0004" : "0 3px 15px #0003",
                    }}
                  >
                    {(() => {
            ...
            // --- CURRENT & RIGHT SIDE CARDS ---
            if (pos >= 0) {
              let scale;
              if (pos === 0) scale = 1.1;
              else if (pos === 1) scale = 1.0;
              else scale = 0.9 - pos * 0.05;

              const zIdx = 100 - pos;
              const yOffset = pos === 0 ? 0 : pos * 6;
              const xOffset = pos * CARD_WIDTH * 0.2;

              const animStyle = isShiftOut
                ? { left: "150%", opacity: 0, transition: "all 1s ease" }
                : {
                    left: "50%",
                    opacity: pos === 0 ? 1 : 0.8,
